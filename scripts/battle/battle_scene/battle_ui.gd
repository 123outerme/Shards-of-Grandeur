...
func should_apply_reward_modifiers() -> bool:
	return playerWins and (PlayerResources.playerInfo.encounter is RandomEncounter)
...
func build_rewards() -> Array[Reward]:
	...
	if len(battleController.state.rewards) == 0:
		var applyRewardModifiers: bool = should_apply_reward_modifiers()
		...
		if PlayerResources.playerInfo.encounter is StaticEncounter and (PlayerResources.playerInfo.encounter as StaticEncounter).useStaticRewards:
			...
			if battleController.enemyCombatant1.combatant != null and not battleController.enemyCombatant1.is_alive():
				...
				if reward != null:
					...
					reward = reward.scale_reward_by_level(
						battleController.enemyCombatant1.initialCombatantLv,
						battleController.enemyCombatant1.combatant.stats.level,
						PlayerResources.playerInfo.combatant.stats.level,
						Reward.CUSTOM_WIN_SCALE if playerWins else Reward.CUSTOM_LOSE_SCALE
			...
			if battleController.enemyCombatant2.combatant != null:
				if not battleController.enemyCombatant2.is_alive():
					...
					if reward != null:
						# if there was a reward, scale it up by the combatant's level
						reward = reward.scale_reward_by_level(
							battleController.enemyCombatant2.initialCombatantLv,
							battleController.enemyCombatant2.combatant.stats.level,
							PlayerResources.playerInfo.combatant.stats.level,
							Reward.CUSTOM_WIN_SCALE if playerWins else Reward.CUSTOM_LOSE_SCALE
						).scale_reward_by_modifiers(expModifier, goldModifier, itemsModifier)
			...
			if battleController.enemyCombatant3.combatant != null:
				if not battleController.enemyCombatant3.is_alive():
					...
					if reward != null:
						# if there was a reward, scale it up by the combatant's level
						reward = reward.scale_reward_by_level(
							battleController.enemyCombatant3.initialCombatantLv,
							battleController.enemyCombatant3.combatant.stats.level,
							PlayerResources.playerInfo.combatant.stats.level,
							Reward.CUSTOM_WIN_SCALE if playerWins else Reward.CUSTOM_LOSE_SCALE
						).scale_reward_by_modifiers(expModifier, goldModifier, itemsModifier)
				# if the enemy was present but not defeated, it explicitly gets null rewards. otherwise gets the found rewards
...
func _on_focus_changed(control: Control):
	if previousFocus != null and control == battlePanels.flowOfBattle.fobButton and previousFocus != battlePanels.flowOfBattle.fobButton \
