...
func load_evolve_results_panel():
	if newEvolution != null:
		...
		details = 'By equipping the ' + newEvolution.requiredEquipment.itemName \
				+ ', ' + combatant.disp_name() + ' has become a ' + newEvolution.stats.displayName + '!'
	else:
		title = combatant.disp_name() + ' Reverted to Base'
		details = 'By unequipping the ' + prevEvolution.requiredEquipment.itemName \
				+ ', ' + combatant.disp_name() + ' has reverted back to base form.'
	
	if combatantLosingEvolving != null:
		details += '\n' + combatantLosingEvolving.disp_name() + ' unequipped the ' + newEvolution.requiredEquipment.itemName + ' and reverted back to base form.'
	...
	if switchEvolutionFlags != 0:
		...
		if (switchEvolutionFlags >> 2) & 1 == 1: # 0b0100
			var str: String = 'It forgot how to use some moves'
			if (switchEvolutionFlags >> 3) & 1 == 1: # 0b1000
				str += ', so it completely changed up its moveset!'
			else:
				str += '.'
			changesStrings.append(str)
